...
<main class="login-page">
    <div class="login-card">
        ...
        <form class="admin-form" method="POST" action="{{ route('technician.login.store') }}">
            @csrf
            <div><label for="email">Technician email</label><input id="email" type="email" name="email" value="{{ old('email') }}" autocomplete="email" required autofocus></div>
            <div><label for="password">Password</label><input id="password" type="password" name="password" autocomplete="current-password" required></div>
